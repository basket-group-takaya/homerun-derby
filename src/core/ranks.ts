...
export const RANKS: readonly Rank[] = ['G', 'F', 'E', 'D', 'C', 'B', 'A', 'S'];

/** The lowest ability value that earns each rank. */
export const RANK_FLOOR: Readonly<Record<Rank, number>> = {
...
export const rankOf = (value: number): Rank => {
  ...
  let out: Rank = 'G';
  for (const r of RANKS) if (v >= (RANK_FLOOR[r] as number)) out = r;
  return out;
};
 ...
 */
export const catchRadiusFor = (meet: number): number =>
  0.045 + (clampAbility(meet) / 100) * 0.080;
 ...
 */
export const maxExitKmhFor = (power: number): number =>
  126 + Math.max(ABILITY_MIN, Math.min(EFFECTIVE_POWER_MAX, power)) * 0.41;
