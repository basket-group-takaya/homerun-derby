...
type Pole = { readonly x: number; readonly phase: number };
 ...
 */
const clothPoint = (
  pole: Pole, u: number, v: number, t: number,
): Vec3 => {
  ...
  return vec(
    ...
    top - v * FLAG_HEIGHT + swing * reach * WAVE_LIFT * (0.55 + 0.75 * v),
    SCOREBOARD_Z + swing * reach * WAVE_AMPLITUDE,
  );
};

/** The pole, drawn whether or not the cloth is ready. */
const drawPole = (
  ctx: CanvasRenderingContext2D, p: Projector, pole: Pole, day: boolean,
): void => {
 ...
 */
const drawCloth = (
  ctx: CanvasRenderingContext2D, p: Projector, pole: Pole,
  logo: HTMLImageElement, t: number, day: boolean,
): void => {
  ...
  for (let i = 0; i < STRIPS; i++) {
    ...
    if (!q0 || !q1 || !q2 || !q3) return;      // any corner behind the eye: skip
  ...
  for (const s of strips) outline.push(s.p0);
  outline.push((strips[strips.length - 1] as Strip).p1);
  for (let i = strips.length - 1; i >= 0; i--) outline.push((strips[i] as Strip).p3);
  outline.push((strips[0] as Strip).p2);
  let ox = 0;
  let oy = 0;
  for (const q of outline) { ox += q.x; oy += q.y; }
  ox /= outline.length;
  oy /= outline.length;
  ctx.beginPath();
  outline.forEach((q, i) => {
  ...
  for (let i = 0; i < STRIPS; i++) {
    ...
    if (!Number.isFinite(ax) || !Number.isFinite(cy)) continue;
...
export const drawFlags = (
  ctx: CanvasRenderingContext2D, p: Projector,
  logo: HTMLImageElement | null, clock: number, day: boolean,
): void => {
  for (const pole of POLES) {
    drawPole(ctx, p, pole, day);
    if (logo && logo.complete) drawCloth(ctx, p, pole, logo, clock, day);
  }
};
