...
export type CrackQuality = 'just' | 'good' | 'poor' | 'jammed' | 'foul';
...
export const createSfx = () => {
  ...
  const noiseBurst = (
    ...
    o: {
      ...
      attack: number; decay: number; sweepTo?: number;
    },
  ): void => {
    if (!master) return;
  ...
  const tone = (
    c: AudioContext, at: number,
    o: { type: OscillatorType; freq: number; to?: number; gain: number; attack: number; decay: number },
  ): void => {
    if (!master) return;
  ...
  return {
    ...
    isReady(): boolean { return ctx !== null && ctx.state === 'running'; },
     ...
     */
    crack(quality: CrackQuality, strength: number): void {
      const c = ensure();
      if (!c) return;
      ...
      const mix: Record<CrackQuality, readonly [number, number, number, number, number]> = {
     ...
     */
    crowd(level: number, seconds: number): void {
      const c = ensure();
      if (!c || !master) return;
      const t = c.currentTime + 0.001;
