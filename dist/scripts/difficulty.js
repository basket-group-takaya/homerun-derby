...
import { PLATE_HALF_WIDTH, PLAYERS, PLAYER_IDS, T_SWING, ZONE_BOTTOM, ZONE_TOP, } from '../src/core/constants.js';
...
const PITCH_CAP = 40;
let cappedRounds = 0;
const inZone = (p) => Math.abs(p.x) <= PLATE_HALF_WIDTH && p.y >= ZONE_BOTTOM && p.y <= ZONE_TOP;
/**
...
const playRound = (player, seed, offsetFrames, mode, cell, undercut, aimError) => {
    let state = initialState(seed, player, mode);
    let guard = 0;
...
for (const player of PLAYER_IDS)
    undercuts.set(player, bestUndercut(player));
...
const num = (n, digits = 1) => n.toFixed(digits);
...
for (const player of PLAYER_IDS) {
    for (const [label] of PRECISIONS) {
        ...
        if (!c)
            continue;
        console.log(`| ${PLAYERS[player].roman} | ${label} | ${pct(c.homeRuns, c.swings)} `
