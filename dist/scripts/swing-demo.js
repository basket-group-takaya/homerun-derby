 ...
 */
const playOne = (seed, player, frameOffset, aimError) => {
    ...
    guard = 0;
    while (s.phase === 'flight' && guard++ < 2000)
        s = step(s, { kind: 'tick', dt: TICK });
    return s;
};
line('='.repeat(78));
...
for (const id of PLAYER_IDS) {
    ...
    for (const spread of [0, 2, 5]) {
        ...
        for (let seed = 1; seed <= 60; seed++) {
            ...
            if (!s.pitch?.intendedStrike)
                continue;
            n++;
            if (s.swing?.field?.outcome === 'homeRun')
                hr++;
        }
...
line();
line('='.repeat(78));
//# sourceMappingURL=swing-demo.js.map