...
import { OUTS_PER_ROUND, PLATE_HALF_WIDTH, PLAYER_IDS, ZONE_BOTTOM, ZONE_TOP, } from '../src/core/constants.js';
import { seedRng, nextRange, pick } from '../src/core/rng.js';
const TICK = 1 / 60;
const finite = (n) => Number.isFinite(n);
/** Everything that must be true of the state after any command, ever. */
const checkInvariants = (s) => {
    if (!finite(s.time) || s.time < 0)
        return `time = ${s.time}`;
    if (!finite(s.cursor.x) || !finite(s.cursor.y))
        return 'cursor is not finite';
    if (Math.abs(s.cursor.x) > PLATE_HALF_WIDTH + 0.5)
        return `cursor x escaped: ${s.cursor.x}`;
    if (s.cursor.y < ZONE_BOTTOM - 0.5 || s.cursor.y > ZONE_TOP + 0.5) {
        return `cursor y escaped: ${s.cursor.y}`;
    }
    if (s.round.outs < 0 || s.round.outs > OUTS_PER_ROUND)
        return `outs = ${s.round.outs}`;
    if (!finite(s.round.score) || s.round.score < 0)
        return `score = ${s.round.score}`;
    if (s.round.homeRuns < 0 || s.round.homeRuns > s.round.swings) {
        return `homeRuns ${s.round.homeRuns} > swings ${s.round.swings}`;
    }
    if (!finite(s.round.longest) || s.round.longest < 0)
        return `longest = ${s.round.longest}`;
    const swing = s.swing;
    if (swing) {
        ...
        if (c.exitVelocity < 0)
            return `negative exit velocity ${c.exitVelocity}`;
        if (swing.field) {
            if (!finite(swing.field.distance))
                return 'distance is not finite';
            if (swing.field.distance < 0)
                return `negative distance ${swing.field.distance}`;
        }
        for (const p of swing.trail) {
            if (!finite(p.x) || !finite(p.y) || !finite(p.z))
                return 'trail point is not finite';
            if (p.y < -1e-6)
                return `ball went below ground: y = ${p.y}`;
        }
        if (!finite(swing.hangTime) || swing.hangTime < 0)
            return `hangTime = ${swing.hangTime}`;
    }
...
const runSeed = (seed, swings, failures) => {
    let rng = seedRng(seed ^ 0x5bf03635);
    const chosen = pick(rng, PLAYER_IDS);
    rng = chosen.rng;
    let state = initialState(seed, chosen.value);
    let rounds = 0;
    let done = 0;
    const guard = (why, swingIndex) => {
    ...
    for (let i = 0; i < swings; i++) {
        ...
        while (state.phase === 'pitching' && steps++ < 400) {
            if (!swung && elapsed >= swingAt) {
                ...
                swung = true;
                if (guard('after swing', i))
                    return { rounds, swingsDone: done };
                continue;
            }
            state = step(state, { kind: 'tick', dt: TICK });
            elapsed += TICK;
            if (guard('mid-pitch tick', i))
                return { rounds, swingsDone: done };
        }
        while (state.phase === 'flight' && steps++ < 3000) {
            state = step(state, { kind: 'tick', dt: TICK });
            if (guard('mid-flight tick', i))
                return { rounds, swingsDone: done };
        }
        if (steps >= 3000) {
...
for (const f of failures)
    console.log(`  seed ${f.seed} swing ${f.swing}: ${f.why}`);
