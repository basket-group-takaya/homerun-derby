 ...
 */
import { simulateBattedBall, spinForContact, dragCoefficient, liftCoefficient } from '../src/core/physics.js';
...
import { PLAYERS, PLAYER_IDS, MAX_EXIT_KMH, BASE_LAUNCH_ANGLE, CURSOR_RADIUS, REFERENCE_AIR_DENSITY, AIR_DENSITY, EXIT_VELOCITY_MAX, FENCE_CENTRE, FENCE_ALLEY, FENCE_LINE, FENCE_HEIGHT, DEFAULT_CONTACT_HEIGHT, } from '../src/core/constants.js';
/** Scripts measure the mid-career batter; level 1 and 99 are different games. */
const LEVEL = 50;
const line = (s = '') => { process.stdout.write(s + '\n'); };
const f = (n, w, d = 1) => n.toFixed(d).padStart(w);
const carry = (kmh, deg, rpm, rho) => simulateBattedBall({
...
line(`     ランク別最大初速: ` + Object.keys(MAX_EXIT_KMH)
    .map((r) => `${r}=${MAX_EXIT_KMH[r]}`).join(' '));
line(`     ランク別カーソル半径: ` + Object.keys(CURSOR_RADIUS)
    .map((r) => `${r}=${CURSOR_RADIUS[r]}`).join(' '));
line(`     弾道別ベース角: ` + Object.keys(BASE_LAUNCH_ANGLE)
    .map((t) => `${t}=${BASE_LAUNCH_ANGLE[t]}°`).join(' '));
line();
line('='.repeat(76));
//# sourceMappingURL=physics-check.js.map