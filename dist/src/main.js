...
import { makeProjector, PITCH_CAMERA, cameraAfterContact, shakeCamera, CUT_HOLD_END, CUT_PULLBACK_END, } from './render/camera.js';
import { drawScene } from './render/scene.js';
import { DELIVERY_SECONDS } from './render/pitcher.js';
import { drawHud, resultCardVisible } from './render/hud.js';
import { cardBoxes, dayBox, drawCutIn, drawShop, drawTitle, modeBox, pitcherBoxes, shopBackBox, shopListBottom, shopListTop, shopOpenBox, shopRows, shopScrollMax, soundBox, } from './render/screens.js';
...
import { BREAK_STARS, SPECIAL_NAME, breakthroughForXp, specialXp, specialsAt, starProgress, starsForXp, } from './core/ability.js';
...
let view = { width: LOGICAL_WIDTH, height: LOGICAL_WIDTH * 16 / 9 };
...
const resize = () => {
    ...
    const scale = Math.min(MAX_RENDER_SCALE, ((drawW * dpr) / view.width) * RENDER_SUPERSAMPLE);
...
const loadSprites = (id) => {
    ...
    for (const pose of POSES) {
        const img = new Image();
        spritesPending++;
        img.onload = () => { spritesPending--; };
        img.onerror = () => { spritesPending--; };
 ...
 */
const logoBack = new Image();
logoBack.src = 'assets/logo_back.png';
// ---------------------------------------------------------------------------
// state
// ---------------------------------------------------------------------------
let state = initialState(Date.now() & 0x7fffffff, 'takaya');
const queue = [];
const send = (c) => { queue.push(c); };
const fx = createFx();
const sfx = createSfx();
/**
...
const slot = (id = selected) => save.players[id];
const levelFor = (id = selected) => levelOf(slot(id).xp);
/**
...
let cutIn = null;
const FACE_VARIANTS = ['bust', 'smile', 'thinking', 'relief', 'serious'];
const faces = {};
for (const id of PLAYER_IDS) {
...
const showCutIn = (variant, caption, colour, life = 1.6) => {
...
export const swingButton = (v, bottomInset) => {
...
const titleTap = (px, py) => {
    ...
    for (const box of cardBoxes(view, insets)) {
        if (px >= box.x && px <= box.x + box.w && py >= box.y && py <= box.y + box.h) {
            ...
            storeSave(save);
            state = initialState(Date.now() & 0x7fffffff, selected, roundMode, me.bat, levelOf(me.xp), me.pitcher, breakthroughForXp(me.xp));
            fx.reset();
            cutIn = null;
            previousEvent = null;
...
const shopTap = (px, py) => {
    ...
    if (dragged > view.width * 0.02)
        return;
    if (py < shopListTop(view, insets) || py > shopListBottom(view, insets))
        return;
...
canvas.addEventListener('pointermove', (e) => {
    if (uiScreen !== 'shop' || !dragFrom)
        return;
    const p = toLogical(e);
    dragged = Math.max(dragged, Math.abs(p.y - dragFrom.y));
    shopScroll = Math.max(0, Math.min(shopScrollMax(view, insets), dragFrom.scroll - (p.y - dragFrom.y)));
});
...
window.addEventListener('keydown', (e) => {
    ...
    if (idx >= 0) {
        ...
        if (id) {
            send({ kind: 'selectPlayer', player: id });
            fx.reset();
        }
    }
});
...
const contactPoint = () => {
    ...
    if (!swing || swing.trail.length < 2)
        return { at, dir: vec(0, 0.4, 1) };
...
const announce = (e) => {
    ...
    sfx.crack(e.grade === 'perfect' ? 'just'
        : e.grade === 'great' ? 'just'
            : e.grade === 'good' ? 'good'
                : e.outcome === 'foul' ? 'foul' : 'poor', Math.min(1, ((swing?.contact.exitVelocity ?? 25) - 22) / 28));
...
const announceLanding = (e) => {
    const swing = state.swing;
    if (!swing || !swing.field)
        return;
    ...
    if (homeRun) {
        ...
        if (e.titanic)
            showCutIn('bust', '特大弾！', '255,150,60', 2.0);
        else if (state.round.streak >= 3)
            showCutIn('bust', `${state.round.streak} 連発`, '255,215,106', 1.7);
        else
            showCutIn('bust', 'ホームラン', '255,215,106', 1.5);
...
const reactToTransitions = () => {
    ...
    if (e && e !== previousEvent) {
        ...
        if (e.outcome === 'homeRun')
            ...
            buzz(34);
        else if (e.outcome === 'inPlay' || e.outcome === 'foul')
            buzz(18);
        else if (e.outcome === 'whiff')
            buzz(9);
...
const sinceContact = () => {
    if (cameraHome)
        return null;
    if (!state.swing || !state.swing.field)
        return null;
...
const batterFade = () => {
    const t = sinceContact();
    if (t === null)
        return 1; // at the plate, or back at it after the cut
    if (t <= CUT_HOLD_END)
        return 1;
...
if (startPlayer && PLAYER_IDS.includes(startPlayer)) {
    state = initialState(Date.now() & 0x7fffffff, startPlayer);
}
if (autoSwing) {
    uiScreen = 'playing';
...
if (devHooks) {
    ...
    window.hrdPlay = (who = 'takaya') => {
        if (!PLAYER_IDS.includes(who))
...
const runAutoSwing = () => {
    if (!autoSwing || state.phase !== 'pitching' || !state.flight)
        return;
...
const advance = (dt) => {
    ...
    if (uiScreen === 'title' || uiScreen === 'shop')
        return;
...
const render = () => {
    if (uiScreen === 'title') {
        drawTitle(ctx, view, insets, faces, selected, roundMode, best, sfx.isMuted(), save, PITCHERS[slot().pitcher]);
        return;
    }
    if (uiScreen === 'shop') {
    ...
    const drawPauseScreen = () => {
        ...
        ctx.fillText(`SCORE ${state.round.score}　HR ${state.round.homeRuns}　OUT ${state.round.outs}/10`, cx, view.height * 0.30 + view.width * 0.075);
        ctx.font = `600 ${view.width * 0.032}px "Segoe UI", system-ui, sans-serif`;
        ctx.fillStyle = 'rgba(160,182,214,0.9)';
        ctx.fillText(`${PLAYERS[state.player].name}　Lv.${levelOf(slot().xp)}${starText(slot().xp)}`
            + `　${BATS[state.bat].name}`, cx, view.height * 0.30 + view.width * 0.125);
        const button = (b, label, primary) => {
    ...
    const drawCoach = () => {
        if (coachLeft <= 0 || uiScreen !== 'playing')
            return;
    ...
    const drawLevelBar = () => {
        if (uiScreen !== 'playing')
            return;
        if (state.phase === 'roundOver')
            return; // the full-screen card owns it all
        if (resultCardVisible(state))
            return; // the card owns this strip
    ...
    drawHud(ctx, state, view, insets, best, lastBanked, slot().xp, uiScreen === 'playing' ? view.width * SWING_BAND : 0, levelUpLeft > 0
        ? {
            level: levelUpTo,
            bats: levelUpBats.map((id) => BATS[id].name),
    ...
    drawPauseScreen();
    if (cutIn)
        drawCutIn(ctx, view, faces, cutIn);
