...
export const createFx = () => {
    ...
    let slow = 1;
    let slowTime = 0;
    const burst = (at, dir, count, speed, colour, opts = {}) => {
        const spread = opts.spread ?? 0.75;
        for (let i = 0; i < count; i++) {
            const s = speed * between(0.35, 1.0);
            particles.push({
                pos: { ...at },
                vel: vec(dir.x * s + between(-spread, spread) * s, dir.y * s + between(-spread, spread) * s + s * 0.25, dir.z * s + between(-spread, spread) * s),
                life: 0,
    ...
    return {
        ...
        impact(p) {
            ...
            if (p.quality === 'just') {
                ...
                flash = 0.9;
                flashColour = '255,244,214';
                shakeAmp = 0.055 + 0.045 * strength;
                burst(p.at, p.dir, 34, 9 + 7 * strength, '255,236,170', { size: 0.020, life: 0.5, drag: 2.2 });
        ...
        whiff(at) {
            ...
            shakeDir = { x: 1, y: 0.2 };
            shakeTime = 0;
            burst(at, vec(0, 0, -1), 6, 2.2, '150,165,190', { size: 0.022, life: 0.26, gravity: 0, drag: 3.0 });
        },
        ...
        land(p) {
            ...
            shakeDir = { x: 0.3, y: -1 };
            shakeTime = 0;
            burst(p.at, vec(0, 1, 0), p.homeRun ? 26 : 14, p.homeRun ? 6 : 3.5, p.homeRun ? '255,225,150' : '190,160,130', { size: 0.11, life: 0.9, spread: 1.0, drag: 1.0 });
            rings.push({
