 ...
 */
const outlined = (ctx, text, x, y, fill, weight) => {
    ctx.lineJoin = 'round';
    ctx.miterLimit = 2;
...
const drawTopBar = (ctx, state, view, insets) => {
    ...
    outlined(ctx, 'SCORE', pad, top + view.width * 0.035, 'rgba(228,240,255,0.95)', view.width * 0.012);
    ctx.font = `800 ${view.width * 0.098}px ${FONT}`;
    outlined(ctx, String(round.score), pad, top + view.width * 0.125, '#ffffff', view.width * 0.022);
    // home runs, right-aligned
    ctx.textAlign = 'right';
    ctx.font = `600 ${view.width * 0.032}px ${FONT}`;
    outlined(ctx, 'HR', view.width - pad, top + view.width * 0.035, 'rgba(228,240,255,0.95)', view.width * 0.012);
    ctx.font = `800 ${view.width * 0.078}px ${FONT}`;
    outlined(ctx, String(round.homeRuns), view.width - pad, top + view.width * 0.118, '#ffd76a', view.width * 0.020);
 ...
 */
const drawPitchChip = (ctx, state, view, insets) => {
 ...
 */
const drawTimingBar = (ctx, state, view, insets) => {
 ...
 */
export const resultCardVisible = (state) => {
...
const drawResult = (ctx, state, view, insets) => {
    ...
    if (e.outcome === 'take' && !e.out && !e.discipline)
        return;
    const pad = view.width * 0.045;
    ...
    ctx.fillText(e.discipline ? '見極め' : GRADE_LABEL[e.grade], pad + view.width * 0.035, y + h * 0.44);
    const outcomeLabel = {
...
const drawRoundOver = (ctx, state, view, best, banked, wallet, levelUp) => {
    ...
    ctx.fillStyle = 'rgba(220,232,250,0.9)';
    ctx.fillText(`ホームラン ${round.homeRuns} 本   最長 ${round.longest.toFixed(1)} m`, cx, y);
...
const drawPlayerChip = (ctx, state, view, insets) => {
    if (resultCardVisible(state))
        return;
    ...
    ctx.fillText(`ミート ${rankOf(a.meet)}${a.meet} / パワー ${rankOf(a.power)}${a.power}`
        + ` / 弾道 ${a.trajectory}`, pad, y + view.width * 0.040);
};
export const drawHud = (ctx, state, view, insets, best, banked, wallet, 
/**
 ...
 */
bottomReserve = 0, levelUp = null) => {
    const lifted = { ...insets, bottom: insets.bottom + bottomReserve };
