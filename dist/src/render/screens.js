 ...
 */
import { PLAYERS, PLAYER_IDS, PLAYER_FLAVOUR, } from '../core/constants.js';
import { batsFor } from '../storage.js';
import { BAT_UNLOCK_LEVEL, levelOf, levelProgress, xpToNext } from '../core/level.js';
import { BREAK_STARS, SPECIAL_IDS, SPECIAL_LEVEL, SPECIAL_NAME, SPECIAL_TIER, abilityAt, breakthroughForXp, starsForXp, } from '../core/ability.js';
...
const ready = (img) => img !== undefined && img.complete && img.naturalWidth > 0;
/** Draw an image cropped to fill a box, like CSS object-fit: cover. */
const drawCover = (ctx, img, x, y, w, h, focusY = 0.42) => {
    const scale = Math.max(w / img.naturalWidth, h / img.naturalHeight);
    const sw = w / scale;
    const sh = h / scale;
    const sx = (img.naturalWidth - sw) / 2;
    const sy = Math.max(0, Math.min(img.naturalHeight - sh, img.naturalHeight * focusY - sh / 2));
    ctx.drawImage(img, sx, sy, sw, sh, x, y, w, h);
};
/** Where the three cards sit. main.ts hit-tests taps against this. */
export const cardBoxes = (view, insets) => {
 ...
 */
export const soundBox = (view, insets) => {
 ...
 */
export const pitcherBoxes = (view, insets) => {
    const pad = view.width * 0.045;
...
const drawCard = (ctx, box, faces, selected, save) => {
    ...
    if (ready(bust))
        drawCover(ctx, bust, box.x, box.y, pw + 2, box.h, 0.36);
    else {
        ctx.fillStyle = '#20304c';
        ctx.fillRect(box.x, box.y, pw, box.h);
    }
    ...
    ctx.fillText(`Lv.${level}`, box.x + box.w - box.h * 0.10 - ctx.measureText(`Lv.${level}`).width, row(0.195));
    /*
    ...
    if (stars > 0) {
        ...
        ctx.fillText(text, box.x + box.w - box.h * 0.10 - ctx.measureText(text).width, row(0.305));
    }
    /*
    ...
    if (owned.length === 0) {
        ...
        ctx.fillText(next ? `Lv.${SPECIAL_LEVEL[next]} で ${SPECIAL_NAME[next]}` : '特殊能力なし', tx, row(0.680));
    }
    else {
        let sx = tx;
        for (const id of owned) {
            ...
            if (sx + w > box.x + box.w - box.h * 0.08)
                break;
...
export const drawTitle = (ctx, view, insets, faces, selected, mode, best, muted, save, pitcher) => {
    ...
    ctx.fillText(save.rounds > 0
        ? `${save.rounds} ラウンド　HR ${save.homeRuns} 本　最長 ${save.bestDistance} m`
        : '選手を選んでください', cx, insets.top + view.height * 0.198);
    if (best > 0) {
    ...
    ctx.fillText(`Lv.${levelOf(save.players[selected].xp)}　　`
        + `バット：${BATS[save.players[selected].bat].name}`, cx, insets.top + view.height * 0.248);
    ...
    if (pb0)
        ctx.fillText('相手ピッチャー', cx, pb0.y - view.width * 0.022);
    ...
    ctx.fillText(mode === 'classic' ? 'ホームラン以外は全部アウト。10アウトで終了'
        : 'ホームラン以外の打球にも点が入る。10アウトで終了', cx, m.y + m.h + view.width * 0.048);
    ctx.textAlign = 'left';
};
/**
 ...
 */
export const drawCutIn = (ctx, view, faces, cut) => {
 ...
 */
export const shopListTop = (view, insets) => insets.top + view.height * 0.185;
export const shopListBottom = (view, insets) => view.height - insets.bottom - view.width * 0.165;
const SHOP_ROW_H = 0.155;
const SHOP_GAP = 0.018;
export const shopRows = (view, insets, scroll = 0) => {
    const pad = view.width * 0.045;
...
export const shopScrollMax = (view, insets) => {
    const h = view.width * SHOP_ROW_H;
    const gap = view.width * SHOP_GAP;
...
export const shopBackBox = (view, insets) => {
...
const drawShopRow = (ctx, row, owned, equipped, affordable) => {
...
export const drawShop = (ctx, view, insets, save, who, notice, scroll = 0) => {
    const g = ctx.createLinearGradient(0, 0, 0, view.height);
    ...
    ctx.font = `800 ${view.width * 0.072}px ${FONT}`;
    ctx.fillStyle = '#ffd76a';
    ctx.fillText(`Lv.${levelOf(save.players[who].xp)}　　`
        + `次まで ${xpToNext(save.players[who].xp).toLocaleString()} EXP`, view.width / 2, insets.top + view.height * 0.148);
    if (notice) {
        ctx.font = `700 ${view.width * 0.032}px ${FONT}`;
        ctx.fillStyle = '#9fe3a6';
    ...
    ctx.save();
    ctx.beginPath();
    ctx.rect(0, shopListTop(view, insets), view.width, shopListBottom(view, insets) - shopListTop(view, insets));
    ctx.clip();
    const owned = batsFor(save, who);
    for (const row of shopRows(view, insets, scroll)) {
        if (row.y + row.h < shopListTop(view, insets))
            continue;
        if (row.y > shopListBottom(view, insets))
            continue;
    ...
    if (scroll < shopScrollMax(view, insets) - 1) {
        ...
        ctx.fillText('▼ 上下にドラッグ', view.width / 2, shopListBottom(view, insets) + view.width * 0.040);
    }
