...
import { ROUND_SIDES, drawOutline, drawQuads, jointQuads, limbQuads, roundLimb, taperQuads, } from './figure.js';
/**
...
const RUBBER = vec(0, 0.254, MOUND_DISTANCE);
const GREY = [214, 216, 224];
const TRIM = [34, 48, 78];
const SKIN = [222, 180, 154];
const CAP = [30, 42, 70];
const SHOE = [28, 32, 44];
const GLOVE = [86, 58, 40];
/** A point in the pitcher's own frame: his right, up, and toward home. */
const pt = (right, up, forward) => vec(RUBBER.x - right, RUBBER.y + up, RUBBER.z - forward);
...
const sample = (keys, t) => {
    ...
    for (let i = 1; i < keys.length; i++) {
        ...
        if (t <= b.at) {
            ...
            return vec(a.v.x + (b.v.x - a.v.x) * f, a.v.y + (b.v.y - a.v.y) * f, a.v.z + (b.v.z - a.v.z) * f);
        }
    }
...
export const pitcherQuads = (windup) => {
    ...
    push(taperQuads(add(strideFoot, vec(0, -0.03, 0.04)), add(strideFoot, vec(0, -0.03, -0.14)), 0.052, 0.040, SHOE, vec(0, 1, 0), 6, 0.6));
    push(taperQuads(add(pivotFoot, vec(0, -0.03, 0.04)), add(pivotFoot, vec(0, -0.03, -0.13)), 0.052, 0.040, SHOE, vec(0, 1, 0), 6, 0.6));
    // ----- trunk
    push(taperQuads(hips, chest, 0.104, 0.138, GREY, vec(1, 0, 0), ROUND_SIDES, 0.76));
    push(taperQuads(chest, add(chest, vec(0, 0.20, 0)), 0.138, 0.082, GREY, vec(1, 0, 0), ROUND_SIDES, 0.76));
    push(roundLimb(shoulderGlove, shoulderThrow, 0.066, GREY, vec(0, 1, 0)));
    // the jersey placket, so the trunk is not one flat slab
    push(limbQuads(add(hips, vec(0, 0.02, -0.096)), add(chest, vec(0, 0.06, -0.104)), 0.020, 0.014, TRIM, vec(0, 1, 0)));
    // ----- head and cap
    push(roundLimb(add(head, vec(0, -0.10, 0)), add(head, vec(0, 0.055, 0)), 0.094, SKIN, vec(1, 0, 0), 0.94));
    push(taperQuads(add(head, vec(0, -0.02, 0)), add(head, vec(0, 0.10, 0)), 0.100, 0.062, CAP, vec(1, 0, 0), ROUND_SIDES, 0.96));
    // peak, pointing at the batter, which is the only thing that shows he is
    // looking this way at all
    push(limbQuads(add(head, vec(0, 0.03, -0.07)), add(head, vec(0, 0.015, -0.21)), 0.078, 0.016, CAP));
    // ----- arms
    ...
    push(roundLimb(add(gloveHand, vec(0, 0, 0.05)), add(gloveHand, vec(0, 0, -0.05)), 0.085, GLOVE));
    return quads;
};
...
export const drawPitcher = (ctx, p, windup) => {
    ...
    if (!foot || !crown || foot.y - crown.y < 8)
        return;
 ...
 */
export const pitcherShoulders = (windup) => {
    const t = Math.min(1, Math.max(0, windup));
    const h = sample(HIPS, t);
    const hips = pt(h.x, h.y, h.z);
    const chest = add(hips, vec(0, 0.34, 0));
...
export const pitcherHand = (windup) => {
...
export const releaseError = () => length(sub(pitcherHand(0.82), RELEASE_POINT));
//# sourceMappingURL=pitcher.js.map