...
import { FENCE_HEIGHT, FOUL_ANGLE, MOUND_DISTANCE, POLE_TOP, SCOREBOARD_BOTTOM, SCOREBOARD_HALF_WIDTH, SCOREBOARD_TOP, SCOREBOARD_Z, FENCE_CENTRE, FENCE_ALLEY, FENCE_LINE, } from '../core/constants.js';
const SKINS = {
    night: {
...
const stroke = (ctx, p, pts, colour, width) => {
...
const CROWD = (() => {
    ...
    for (let row = 0; row < rows; row++) {
        ...
        for (let i = 0; i < perRow; i++) {
            ...
            if (r() < 0.16)
                continue; // empty seats, so it is not a solid wall
            const clamped = Math.max(-FOUL_ANGLE, Math.min(FOUL_ANGLE, a));
            const d = fenceDistance(clamped) + out;
            people.push({
                pos: vec(d * Math.sin(radians(a)), y + (r() - 0.5) * 0.25, d * Math.cos(radians(a))),
...
const SKYLINE = (() => {
    ...
    for (let a = -FOUL_ANGLE - 10; a <= FOUL_ANGLE + 10; a += 3.4 + draw() * 2.2) {
        ...
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < 3; c++) {
                if (draw() < 0.42)
                    windows.push({ u: (c + 0.5) / 3, v: (r + 0.5) / rows });
            }
        }
        out.push({
            angle: a,
...
const drawSkyline = (ctx, p) => {
    for (const b of SKYLINE) {
        ...
        const mix = (c, sky) => Math.round(Math.max(0, Math.min(255, c + (sky - c) * haze)));
        const sky = LIGHTS_ON ? [42, 62, 108] : [138, 182, 226];
        fill(ctx, p, face, `rgb(${mix(tone + 6, sky[0])},`
            + `${mix(tone + 10, sky[1])},${mix(tone + 20, sky[2])})`);
        if (!LIGHTS_ON)
            continue; // daylight: no lit windows
...
const drawStands = (ctx, p) => {
    ...
    for (let a = -FOUL_ANGLE; a <= FOUL_ANGLE; a += 1.5)
        rail.push(onFence(a, FENCE_HEIGHT + 0.7, 2.6));
    stroke(ctx, p, rail, 'rgba(150,168,196,0.35)', 1.5);
};
const drawFloodlights = (ctx, p) => {
...
const drawScoreboard = (ctx, p, lit) => {
    ...
    const at = (u, v, out = 0.05) => vec(-hw + 2 * hw * u, bottom + height * v, SCOREBOARD_Z - out);
    const panel = (u0, v0, u1, v1, out = 0.05) => [at(u0, v0, out), at(u1, v0, out), at(u1, v1, out), at(u0, v1, out)];
    const face = [
    ...
    stroke(ctx, p, [...face, face[0]], lit ? '#ffd76a' : '#39445c', 3);
    ...
    const rows = [[0.55, 0.75], [0.32, 0.52]];
    ...
    const lamps = [[3, 0.205], [2, 0.155], [3, 0.105]];
...
const drawFence = (ctx, p) => {
    ...
    for (let a = ADS_FROM; a + SPAN <= ADS_TO; a += SPAN + 0.5) {
        ...
        fill(ctx, p, panel, BOARD_COLOURS[index % BOARD_COLOURS.length]);
...
export const drawPitcher = (ctx, p, windup) => {
    ...
    limb({ x: hipX, y: hipY }, { x: x - h * 0.055 - whip * h * 0.05, y: y - h * 0.24 }, backFoot, h * 0.062);
    // front leg (his left, our right): lifts, then strides toward the plate
    const lift = Math.sin(Math.min(1, t / 0.55) * Math.PI) * (1 - whip * 0.7);
    limb({ x: hipX, y: hipY }, { x: hipX + h * (0.05 + 0.06 * whip), y: hipY - h * 0.13 * lift + h * 0.11 * whip }, { x: hipX + h * (0.07 + 0.20 * whip), y: y - h * 0.40 * lift }, h * 0.062);
    ...
    limb({ x: shoulderX + shoulderHalf * 0.6, y: shoulderY + h * 0.008 }, { x: shoulderX + h * (gloveOut * 0.9), y: shoulderY + h * 0.06 }, glove, h * 0.045);
    ...
    limb({ x: shoulderX - shoulderHalf * 0.6, y: shoulderY + h * 0.008 }, {
...
const paintStadium = (ctx, p, view, flags, when) => {
    applySkin(when);
    drawSky(ctx, view);
    drawSkyline(ctx, p);
    drawStands(ctx, p);
...
let cacheKey = '';
export const drawStadium = (ctx, p, view, flags, when = 'night') => {
