...
import { ROUND_SIDES, drawGroundShadow, drawOutline, drawQuads, jointQuads, limbQuads, roundLimb, taperQuads, yawAbout, } from './figure.js';
/**
...
export const STANCE = vec(-0.72, 0, -0.04);
/**
...
const HAIR_BLACK = [26, 26, 32];
const HAIR_BROWN = [86, 58, 38];
...
const lerpVec = (a, b, f) => vec(a.x + (b.x - a.x) * f, a.y + (b.y - a.y) * f, a.z + (b.z - a.z) * f);
/** Interpolate the keyframes. `progress` below 0 means "not swinging". */
export const batterPose = (progress) => {
...
const lerpTo = (a, b, f) => vec(a.x + (b.x - a.x) * f, a.y + (b.y - a.y) * f, a.z + (b.z - a.z) * f);
/** An elbow placed off the shoulder-hand line, so the arm reads as jointed. */
const elbow = (shoulder, hand, out, bend) => {
 ...
 */
export const chestDirection = (pose) => normalize(yawAbout(vec(1, 0, 0), vec(0, 0, 0), pose.yaw));
/**
 ...
 * doll being spun.
 */
export const batterQuads = (pose, look = LOOKS.takaya) => {
    const quads = [];
    const centre = vec(0, 0, 0);
    const place = (local, share = 1) => add(STANCE, yawAbout(local, centre, pose.yaw * share));
    /**
    ...
    const placeHead = (local) => add(STANCE, yawAbout(local, centre, headYaw));
    /**
    ...
    const placeAt = (local, yaw) => add(STANCE, yawAbout(local, centre, yaw));
    const push = (built) => { quads.push(...built); };
    ...
    push(taperQuads(lerpTo(kneeL, hipL, -0.16), hipL, 0.070, 0.084, PANTS, vec(0, 1, 0), ROUND_SIDES));
    push(taperQuads(lerpTo(kneeR, hipR, -0.16), hipR, 0.070, 0.084, PANTS, vec(0, 1, 0), ROUND_SIDES));
    push(taperQuads(add(footL, vec(-0.03, -0.032, 0)), add(footL, vec(0.15, -0.032, 0)), 0.052, 0.038, look.shoe, vec(0, 1, 0), 10, 0.58));
    push(taperQuads(add(footR, vec(-0.03, -0.032, 0)), add(footR, vec(0.15, -0.032, 0)), 0.052, 0.038, look.shoe, vec(0, 1, 0), 10, 0.58));
    // ----- pelvis and torso
    ...
    push(taperQuads(add(pelvis, vec(0, -0.075, 0)), add(pelvis, vec(0, 0.06, 0)), 0.108, 0.104, PANTS, vec(1, 0, 0), ROUND_SIDES, 0.74));
    push(taperQuads(add(pelvis, vec(0, 0.05, 0)), chest, 0.104, 0.144, NAVY, vec(1, 0, 0), ROUND_SIDES, 0.74));
    ...
    push(taperQuads(add(pelvis, vec(0, 0.030, 0)), add(pelvis, vec(0, 0.075, 0)), 0.107, 0.109, BELT, vec(1, 0, 0), 8, 0.74));
    ...
    push(roundLimb(place(vec(0, SHOULDER_Y, -halfSpan)), place(vec(0, SHOULDER_Y, halfSpan)), 0.072 * look.build, NAVY, vec(1, 0, 0)));
    ...
    push(taperQuads(place(vec(0, SHOULDER_Y + 0.040, 0)), place(vec(0, SHOULDER_Y + 0.062, 0)), 0.086, 0.080, TRIM_ORANGE, vec(1, 0, 0), ROUND_SIDES, 0.74));
    ...
    push(roundLimb(placeHead(vec(0, HEAD_Y - 0.115, 0)), placeHead(vec(0, HEAD_Y + 0.05, 0)), 0.095, look.skin, vec(1, 0, 0), 0.92));
    /**
    ...
    push(taperQuads(placeHead(vec(0, HEAD_Y + 0.012, 0)), placeHead(vec(0, HEAD_Y + 0.062, 0)), 0.100, 0.092, look.hair, vec(1, 0, 0), ROUND_SIDES, 0.96));
    push(taperQuads(placeHead(vec(0, HEAD_Y + 0.062, 0)), placeHead(vec(0, HEAD_Y + 0.108, 0)), 0.092, 0.050, look.hair, vec(1, 0, 0), ROUND_SIDES, 0.94));
    // the back of the head, offset away from the face so the front stays skin
    push(taperQuads(placeHead(vec(0, HEAD_Y - 0.075, -0.040)), placeHead(vec(0, HEAD_Y + 0.045, -0.034)), 0.070, 0.086, look.hair, vec(1, 0, 0), ROUND_SIDES, 0.90));
    // the neck, so the head does not sit straight on the shoulders
    push(taperQuads(place(vec(0, SHOULDER_Y + 0.02, 0)), placeHead(vec(0, HEAD_Y - 0.10, 0)), 0.052, 0.058, look.skin, vec(1, 0, 0), ROUND_SIDES));
    const SPIKES = [
    ...
    for (const s of SPIKES) {
        ...
        const tipAt = placeHead(vec(s.x - 0.030, HEAD_Y + 0.090 + s.len, s.z + (s.z >= 0 ? 0.022 : -0.022)));
    ...
    for (const side of [-1, 1]) {
        push(limbQuads(placeHead(vec(0.072 * side, HEAD_Y + 0.030, 0.004)), placeHead(vec(0.070 * side, HEAD_Y - 0.045, -0.006)), 0.030, 0.034, look.hair, vec(0, 1, 0)));
        push(roundLimb(placeHead(vec(0.086 * side, HEAD_Y - 0.012, -0.010)), placeHead(vec(0.098 * side, HEAD_Y - 0.012, -0.010)), 0.030, look.skin, vec(0, 1, 0), 0.72));
    }
    /**
     ...
     */
    push(taperQuads(placeHead(vec(0.004, HEAD_Y + 0.005, 0.072)), placeHead(vec(0.004, HEAD_Y - 0.012, 0.116)), 0.026, 0.013, look.skin, vec(0, 1, 0), 6));
    // chin, so the jaw has a front edge in profile
    push(taperQuads(placeHead(vec(0.004, HEAD_Y - 0.088, 0.010)), placeHead(vec(0.004, HEAD_Y - 0.072, 0.068)), 0.050, 0.030, look.skin, vec(0, 1, 0), 6, 0.8));
    if (look.glasses) {
        ...
        const GLASS = [42, 44, 52];
        for (const side of [-1, 1]) {
            push(limbQuads(placeHead(vec(0.070 * side, HEAD_Y + 0.020, -0.052)), placeHead(vec(0.052 * side, HEAD_Y + 0.014, 0.062)), 0.008, 0.017, GLASS, vec(0, 1, 0)));
            push(limbQuads(placeHead(vec(0.052 * side, HEAD_Y + 0.014, 0.062)), placeHead(vec(0.008 * side, HEAD_Y + 0.012, 0.082)), 0.008, 0.017, GLASS, vec(0, 1, 0)));
        }
    }
    // ----- arms to the hands
    ...
    push(roundLimb(add(hands, vec(0, 0, -0.038)), add(hands, vec(0, 0, 0.038)), 0.042, GLOVE));
...
const drawDecal = (ctx, p, panel, art) => {
    ...
    const normal = normalize(cross(sub(panel[1], panel[0]), sub(panel[2], panel[1])));
    if (dot(normal, sub(mid, p.eye)) >= 0)
        return;
    const projected = panel.map((v) => p.project(v));
    if (projected.some((q) => q === null))
        return;
    const [tl, tr, br] = projected;
    if (!tl || !tr || !br)
        return;
...
export const drawBatter = (ctx, p, view) => {
...
//# sourceMappingURL=batter.js.map