...
import { BALL_RADIUS, FENCE_ALLEY, FENCE_CENTRE, FENCE_HEIGHT, FENCE_LINE, FOUL_ANGLE, POLE_RADIUS, POLE_TOP, SCOREBOARD_BOTTOM, SCOREBOARD_HALF_WIDTH, SCOREBOARD_TOP, SCOREBOARD_Z, } from './constants.js';
/** Spray angle of a point, in degrees from centre field toward first base. */
export const sprayAngleOf = (p) => degrees(Math.atan2(p.x, p.z));
/**
 ...
 */
export const fenceDistance = (sprayAngleDeg) => {
...
export const isFair = (sprayAngleDeg) => Math.abs(sprayAngleDeg) <= FOUL_ANGLE;
/** Foul-pole base positions, on the fence line at +-FOUL_ANGLE. */
export const POLE_POSITIONS = {
    leftPole: vec(-FENCE_LINE * Math.sin(radians(FOUL_ANGLE)), 0, FENCE_LINE * Math.cos(radians(FOUL_ANGLE))),
    rightPole: vec(FENCE_LINE * Math.sin(radians(FOUL_ANGLE)), 0, FENCE_LINE * Math.cos(radians(FOUL_ANGLE))),
};
/**
 ...
 */
const segmentHitsPole = (a, b, pole) => {
...
const segmentHitsScoreboard = (a, b) => {
    if ((a.z - SCOREBOARD_Z) * (b.z - SCOREBOARD_Z) > 0)
        return false;
    const span = b.z - a.z;
 ...
 */
export const hitTarget = (trail) => {
    for (let i = 1; i < trail.length; i++) {
        const a = trail[i - 1];
        const b = trail[i];
        if (!a || !b)
            continue;
        if (segmentHitsPole(a, b, POLE_POSITIONS.leftPole))
            return 'leftPole';
        if (segmentHitsPole(a, b, POLE_POSITIONS.rightPole))
            return 'rightPole';
        if (segmentHitsScoreboard(a, b))
            return 'scoreboard';
    }
    return null;
};
export const bleacherOf = (sprayAngleDeg) => sprayAngleDeg < -15 ? 'left' : sprayAngleDeg > 15 ? 'right' : 'centre';
/**
 ...
 */
export const judgeBattedBall = (trail, landing, distance) => {
