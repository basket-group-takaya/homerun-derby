 ...
 */
import { vec, add, scale, dot, cross, length, normalize, addScaled, clamp, radians } from './vec.js';
import { AIR_DENSITY, BALL_AREA, BALL_MASS, BALL_RADIUS, CD_BASE, CD_PER_KRPM, CL_A, CL_B, CL_NUM, DT, GRAVITY, MAX_FLIGHT_TIME, SIDESPIN_BASE, SIDESPIN_MAX, SIDESPIN_PER_DEG_SPRAY, SPIN_BASE, SPIN_MAX, SPIN_MIN, SPIN_PER_DEG_LAUNCH, } from './constants.js';
const RPM_TO_RAD = (2 * Math.PI) / 60;
/** Cd = 0.3008 + 0.0292 * (rpm / 1000). Drag rises with spin. */
export const dragCoefficient = (totalRpm) => CD_BASE + CD_PER_KRPM * (Math.abs(totalRpm) / 1000);
/** Cl = 1.120 S / (0.583 + 2.333 S), where S = r |omega_perp| / |v|. */
export const liftCoefficient = (spinParameter) => {
 ...
 */
export const spinForContact = (launchAngleDeg, sprayAngleDeg) => ({
    backspinRpm: clamp(SPIN_BASE + SPIN_PER_DEG_LAUNCH * launchAngleDeg, SPIN_MIN, SPIN_MAX),
    sidespinRpm: -Math.min(SIDESPIN_BASE + SIDESPIN_PER_DEG_SPRAY * Math.abs(sprayAngleDeg), SIDESPIN_MAX),
});
/**
 ...
 */
const spinVector = (sprayAngleDeg, spin) => {
...
const acceleration = (v, omega, cd, rho) => {
 ...
 * docs/SPEC.md 6-1.
 */
export const simulateBattedBall = (input) => {
    const theta = radians(input.launchAngle);
    const phi = radians(input.sprayAngle);
    const spin = { backspinRpm: input.backspinRpm, sidespinRpm: input.sidespinRpm };
    ...
    let v = vec(input.exitVelocity * Math.cos(theta) * Math.sin(phi), input.exitVelocity * Math.sin(theta), input.exitVelocity * Math.cos(theta) * Math.cos(phi));
    const trail = [p];
    let t = 0;
    let apex = p.y;
    while (t < MAX_FLIGHT_TIME) {
        ...
        if (nextP.y <= 0) {
            ...
            const landing = vec(p.x + (nextP.x - p.x) * f, 0, p.z + (nextP.z - p.z) * f);
            trail.push(landing);
            return {
        ...
        if (trail.length === 0 || Math.round(t / DT) % 4 === 0)
            trail.push(p);
    }
