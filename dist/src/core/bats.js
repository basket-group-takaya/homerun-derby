...
export const BAT_IDS = Object.keys(BATS).sort((a, b) => BATS[a].level - BATS[b].level);
export const DEFAULT_BAT = 'wood';
export const isBatId = (v) => typeof v === 'string' && BAT_IDS.includes(v);
/**
 ...
 * else.
 */
export const effectiveMaxExit = (power, bat, specialExit = 1, ceiling = EXIT_VELOCITY_MAX) => Math.min((maxExitKmhFor(power) / 3.6) * bat.exit, ceiling / Math.max(1, specialExit));
/** Experience banked from a finished round. */
export const bankedPoints = (roundScore, bat, extra = 1) => Math.max(0, Math.round(roundScore * bat.points * extra));
/** How much distance a bat is worth [m], for the shelf copy. Rough, and labelled so. */
export const approximateMetres = (bat) => Math.round((bat.exit - 1) * 190);
