...
export const levelOf = (xp) => {
    ...
    while (l < MAX_LEVEL && total >= xpToReach(l + 1))
        l++;
    while (l > 1 && total < xpToReach(l))
        l--;
    return l;
};
 ...
 */
export const BAT_UNLOCK_LEVEL = Object.fromEntries(BAT_IDS.map((id) => [id, BATS[id].level]));
/** Every bat unlocked at or below this level. */
export const unlockedBats = (level) => BAT_IDS.filter((id) => BATS[id].level <= level);
/** The bats granted by crossing from one level to another. Usually none. */
export const batsGained = (before, after) => BAT_IDS.filter((id) => BATS[id].level > before && BATS[id].level <= after);
