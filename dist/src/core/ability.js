 ...
 */
export const POWER_LINE = ['powerHitter', 'artist', 'kingOfOne'];
/**
 ...
 */
export const TRAJECTORY_LEVELS = [1, 12, 34, 62];
export const trajectoryAt = (level) => {
    let out = 1;
    for (const [i, need] of TRAJECTORY_LEVELS.entries())
        if (level >= need)
            out = i + 1;
    return Math.max(1, Math.min(4, out));
};
...
export const SPECIAL_TIER = {
    powerHitter: 'blue',
    wideAngle: 'blue',
    artist: 'gold',
    kingOfOne: 'crown',
};
export const SPECIAL_IDS = ['powerHitter', 'wideAngle', 'artist', 'kingOfOne'];
/**
...
export const lateShapeFor = (specials) => (specials.includes('wideAngle') ? WIDE_ANGLE_LATE_SHAPE : 1);
// ---------------------------------------------------------------------------
 ...
 */
export const idealLaunchAngle = (exitKmh) => {
 ...
 */
export const liftedLaunchAngle = (base, specials, exitKmh = 155) => {
...
export const specialLift = (specials, base = 17, exitKmh = 155) => liftedLaunchAngle(base, specials, exitKmh) - base;
/**
 ...
 */
export const specialExit = (_specials) => 1;
/** Experience multiplier from the specials. Only the crown pays. */
export const specialXp = (specials) => (specials.includes('kingOfOne') ? 1.35 : 1);
// ---------------------------------------------------------------------------
// the whole ability set
// ---------------------------------------------------------------------------
/** Everything a player can do, at a given level. */
export const abilityAt = (player, level, breakthrough = 0) => {
    const l = Math.max(1, Math.min(MAX_LEVEL_ABILITY, Math.floor(level)));
    const f = growth(l);
...
export const abilityForXp = (player, xp) => abilityAt(player, levelOf(xp), breakthroughForXp(xp));
/** The letters, for the select screen. */
export const abilityRanks = (a) => ({
    meet: rankOf(a.meet),
...
export const breakthroughForXp = (xp) => {
...
export const starsForXp = (xp) => Math.floor(breakthroughForXp(xp) * BREAK_STARS + 1e-9);
/** Progress through the current star, 0..1, for the level bar past 99. */
export const starProgress = (xp) => {
