...
import {
  MOUND_DISTANCE, PLATE_HALF_WIDTH, ZONE_BOTTOM, ZONE_TOP, FENCE_CENTRE, FENCE_HEIGHT,
...
const points: readonly (readonly [string, ReturnType<typeof vec>])[] = [
...
const report = (name: string, camera: Camera): void => {
  ...
  for (const [label, v] of points) {
    const s = p.project(v);
    if (!s) { console.log(`  ${label.padEnd(20)}  (behind camera)`); continue; }
    const off = s.x < 0 || s.x > VIEW.width || s.y < 0 || s.y > VIEW.height ? '  <- off screen' : '';
    console.log(
      `  ${label.padEnd(20)} ${s.x.toFixed(1).padStart(9)} ${s.y.toFixed(1).padStart(11)}`
...
if (arg >= 0) {
  ...
  if (n.length >= 7 && n.every((v) => Number.isFinite(v))) {
    const base = {
      eye: vec(n[0] as number, n[1] as number, n[2] as number),
      target: vec(n[3] as number, n[4] as number, n[5] as number),
      vfov: n[6] as number,
    };
    report('--try', (n.length >= 8 ? { ...base, hfov: n[7] as number } : base) satisfies Camera);
  } else {
